...
if (!("kmeans_tb" %in% ls())) {
  ...
  kmeans_tb = read.csv(pipe("unpigz --stdout -p 12 ../data/clusters_kmeans_prescriptions_2018-04-26.gz"), stringsAsFactors=FALSE) %>% tbl_df
...
if (!("specialties_tb" %in% ls())) {
  ...
  specialties_tb = specialties_tb %>% mutate(specialty_description = as.character(specialty_description))
  specialties_tb[specialties_tb$specialty_description == "Student in an Organized Health Care Education/Training Program","specialty_description"] = "Student"
}
...
make_grid = function(counts_tb, n, value.var, include_count) {
  ...
  cn = as.character(1:n)
  for (c in cn) {
    lab = top3_label_grid_tb[[c]]
    lab = if_else(lab == "Physical Medicine and Rehabilitation", "Physical Med/Rehab",
          if_else(lab == "Nurse Practitioner", "Nurse Prac",
          if_else(lab == "Physician Assistant", "Phys Asst",
          if_else(lab == "Internal Medicine", "Internal Med",
          if_else(lab == "Oral Surgery (dentists only)", "Oral Surgery",
          lab)))))

    if (include_count) {
      rounded_count = round(top3_count_grid_tb[[c]] / 1000, 0)
      rounded_count = if_else(rounded_count == 0, "<1k", sprintf("%dk", rounded_count))
      top3_grid_tb[[c]] = sprintf("%s (%s)", lab, rounded_count)
    } else {
      top3_grid_tb[[c]] = sprintf("%s", lab, top3_count_grid_tb[[c]])
    }
  }
...
hcpcs_grid_tb = make_grid(hcpcs_counts_tb, 3, "HCPCS.Description", TRUE)
cluster_to_specialty_grid_tb = make_grid(cluster_to_specialty_counts_tb, 3, "specialty_description", TRUE)
kmeans_to_specialty_grid_tb = make_grid(kmeans_to_specialty_counts_tb, 3, "specialty_description", TRUE)
cluster_to_specialty_grid_tb_nocounts = make_grid(cluster_to_specialty_counts_tb, 3, "specialty_description", FALSE)
kmeans_to_specialty_grid_tb_nocounts = make_grid(kmeans_to_specialty_counts_tb, 3, "specialty_description", FALSE)
...
cluster_to_specialty_grid_tb_nocounts = cluster_to_specialty_grid_tb_nocounts %>% filter(!is.na(cluster))
kmeans_to_specialty_grid_tb_nocounts = kmeans_to_specialty_grid_tb_nocounts %>% filter(!is.na(cluster))
...
print_grid = function(grid_tb) {
  ...
  cat("Cluster & Specialty 1 (Count) & Specialty 2 (Count) & Specialty 3 (Count) \\\\ \n")
  cat("\\hline\n")
  for (i in 1:nrow(grid_tb)) {
    ...
    for (i in 1:3) {
      if (row[i+1] == "NA (NA)") {
        ...
        cat(sprintf(" & %s", str_replace(row[i+1], "&", "\\\\&")))
      }
    }
    cat("\\\\ \n")
  }
...
### Print grids (compact).
print_grid_compact = function(grid_tb, title) {
  #grid_tb = cluster_to_specialty_grid_tb
  #cat("\\begin{tabular}{l|l}\n")
  cat("\\begin{tabular}{l}\n")
  cat("\\hline\n")
  #cat(sprintf("\\multicolumn{2}{c}{%s} \\\\", title))
  cat(sprintf("\\multicolumn{1}{c}{%s} \\\\", title))
  cat("\\hline\n")
  #cat("cluster & First & Second & Third & Fourth & Fifth \\\\ \n")
  #cat("Cluster & Most Common Specialty (Count) & Second Most Common Specialty (Count) & Third Most Common Specialty (Count) \\\\ \n")
  #cat("Cluster & Top 3 Specialties (Counts) \\\\ \n")
  cat("Top 3 Specialties (Counts) \\\\ \n")
  cat("\\hline\n")
  for (i in 1:nrow(grid_tb)) {
    row = grid_tb[i,]
    #cat(sprintf("%s & ", row$cluster))
...
print_grid_compact(cluster_to_specialty_grid_tb, "(a) Our Clustering")
cat("\\quad\n")
print_grid_compact(kmeans_to_specialty_grid_tb, "(b) K-Means Clustering")
cat("}")
cat("\\end{tiny}\n")

### Print grids (compacter).
print_grid_compacter = function(grid_tb, title) {
  #grid_tb = cluster_to_specialty_grid_tb
  cat("\\begin{tabular}{l}\n")
  cat(sprintf("\\multicolumn{1}{c}{%s (Top Three Specialties)} \\\\", title))
  cat("\\hline\n")
  for (i in 1:nrow(grid_tb)) {
    row = grid_tb[i,]
    #cat(sprintf("%s & ", row$cluster))
...
cat("\\makebox[\\textwidth][c]{")
print_grid_compacter(cluster_to_specialty_grid_tb_nocounts, "(a) Our Clustering")
cat("\\quad\n")
print_grid_compacter(kmeans_to_specialty_grid_tb_nocounts, "(b) K-Means Clustering")
cat("}")
cat("\\end{scriptsize}\n")

### Print grids (compacterer).
print_grid_compacterer = function(grid_tb, title) {
  #grid_tb = cluster_to_specialty_grid_tb
  #cat("\\begin{tabular}{l|l}\n")
  cat("\\begin{tabular}{l}\n")
  #cat("\\hline\n")
  #cat(sprintf("\\multicolumn{2}{c}{%s} \\\\", title))
  cat(sprintf("\\multicolumn{1}{l}{\\underline{%s (Top 3 Specialties)}} \\\\", title))
  #cat("\\hline\n")
  #cat("cluster & First & Second & Third & Fourth & Fifth \\\\ \n")
  #cat("Cluster & Most Common Specialty (Count) & Second Most Common Specialty (Count) & Third Most Common Specialty (Count) \\\\ \n")
  #cat("Cluster & Top 3 Specialties (Counts) \\\\ \n")
...
cat("\\begin{scriptsize}")
print_grid_compacterer(cluster_to_specialty_grid_tb, "\\textbf{(a)} Our Clustering")
cat("\\vskip 0.5em\n")
print_grid_compacterer(kmeans_to_specialty_grid_tb, "\\textbf{(b)} K-Means Clustering")
cat("\\end{scriptsize}\n")
