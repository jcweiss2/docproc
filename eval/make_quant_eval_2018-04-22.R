...
#space_tag = "output"
date_tag = "2018-04-22"
tag = sprintf("space_%s.dist_%s.date_%s", space_tag, dist_tag, date_tag)
...
if (!("hidden_tb" %in% ls())) {
  ...
  hidden_tb = read.csv(pipe("unpigz --stdout -p 12 ../data/180422npihidden40.csv.gz"), colClasses=col_classes) %>% tbl_df
