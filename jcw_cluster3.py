...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.leaky_relu(self.map4(F.tanh(x)),1e-1)
...
c_optimizer = optim.Adam(Clu.parameters(), lr=c_learning_rate, weight_decay=1e-4)
...
for epoch in range(num_epochs):
    ...
    for i0, i1 in arangeIntervals(data_size, minibatch_size):
        ...
        if np.random.uniform() < pr_g_update:
            ...
            g_loss = torch.max(g_norms,1-torch.log(g_norms+1e-10)).mean()
            # s_loss = 1e-1*torch.pow(batch_cosine(hidden).mean() - 0.5,2)  # spread out (batch)
            s_loss = tzero
            # g_loss += s_loss
            # g_loss.backward(retain_graph=True)  # incorporate into o_optimizer insead.
            # g_optimizer.step()
            gcounter += 1
        else:
        ...
        if not c_only:
            output = Out(hidden)
            o_loss = MSE(output, batch_mydata)
            e_loss = 1e-1 * Enf(hidden, batch_mydata)  # Enforce H and O similarity
            o_loss += e_loss
            o_loss += g_loss * g_o_ratio
            o_loss.backward()
            o_optimizer.step()
        ...
        if np.random.uniform() < pr_c_update:
            ...
            c_e_loss = ((clusters*torch.log(clusters+1e-10)).mean()+1./minibatch_size).pow(2)  # Match on desired entropy
            c_loss += c_e_loss
            c_loss.backward()
            c_optimizer.step()
            ccounter += 1
        ...
        epoch_losses += torch.cat((g_loss, s_loss, o_loss, e_loss, c_loss))
...
for i0, i1 in arangeIntervals(data_size, 100):
    assignments[i0:i1] = np.argmax(Clu(Gen(mynoise[i0:i1].cuda())).cpu().data.numpy(), axis=1)
    # for i0, i1 in arangeIntervals(data_size, minibatch_size):
...
npidf = pd.DataFrame({'npi':mydatadf['npi'],
                      'cluster':assignments})
npidf.to_csv('180416clusters.csv')
...
npihiddendf.to_csv('180416npihidden.csv')
