...
o_output_size = mydataset[0].shape[1]  # a sample is a set of (e)xamples of length (k) [e x k]
...
g_input_size = mydataset[0].shape[1]
...
for epoch in range(num_epochs):
    ...
    for i0, load_batch_mydata in enumerate(dataloader):
        ...
        for mbi in np.arange(((load_batch_mydata.size()[0]-1) // minibatch_size)+1):
            ...
            if mbi % mbi_print_interval == 0:
                ...
                print("%s: [H: %8.6f;  s: %8.6f]; [O: %8.6f; e: %8.6f]; C: %8.6f" %
...
prefix = 'procdrug' + str(dt.datetime.now()) + \
         '_centroids' + str(desired_centroids) + \
         '_samples' + str(data_size) + \
         '_dims' + str(mynoise.size()[1]) + \
         '_'
npis = []
for p in np.arange(len(mydataset)):
    npis.append(mydataset.__getitem__(p, skip=2, zero_column_long=True))
npis2 = torch.cat(npis).long().cpu().numpy()
pd.DataFrame({'npi':npis2,'cluster':assignments}).to_csv('outputs/'+prefix+str(dt.datetime.now()))
